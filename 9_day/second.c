...
unsigned long particular_rules(int n_players, unsigned long max_value){

    max_value *= 100;

    std::list<unsigned long> current;
    current.push_back( 0 );
    ...
    unsigned long scores[n_players];
    for( int i=0; i < n_players; i++ )
        scores[i] = 0;

    auto c = current.begin();
    for( unsigned long marble = 1; marble < max_value + 1; ++marble ){
...
int read_input(){
    ...
    while( ( read = getline(&ln, &len, fp) ) != -1 ){
        ...
        printf(" Got values: %d and %d. And solution is %lu \n", n_players, max_value, particular_rules( n_players, (unsigned long) max_value ) );
    }
